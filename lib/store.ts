...
export type EventKind = 'event' | 'question';
...
function randomNear(c: { lat: number; lng: number }, radiusKm = 3) {
...
const SEED_ATTENDEES: Attendee[] = [
  // each event creator is auto-attendee
  ...SEED_EVENTS.map((e) => ({
    event_id: e.id,
    user_id: e.host_id,
...
type Actions = {
  signUp: (email: string, _password: string, displayName: string) => string;
  signIn: (email: string, _password: string) => string | null;
  signInGoogle: () => string;
  signOut: () => void;

  setCity: (userId: string, city: CityId) => void;
  joinWaitlist: (userId: string, city: CityId) => void;
  ...
  sendMessage: (eventId: string, userId: string, body: string) => void;
};
  ...
  persist(
    (set, get) => ({
      ...
      signIn: (email, _password) => {
        // demo: match by email-prefix being the display_name (case-insensitive)
...
export function attendeeCount(attendees: Attendee[], eventId: string): number {
  let n = 0;
...
export function isExpired(event: LinkEvent, nowMs: number = Date.now()): boolean {
