...
import { type Coords, formatDistance, haversineKm } from '@/lib/geo';
import { type LinkEvent, type Profile, type Attendee, attendeeCount } from '@/lib/store';
...
export function EventCard({ event, host, attendees, userCoords, unit, now }: Props) {
  ...
  if (Platform.OS === 'web') {
    return (
      <View
        ...
        style={{ backdropFilter: 'blur(12px)' } as never}
      >
        {children}
      </View>
    );
